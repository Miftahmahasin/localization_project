#!/usr/bin/env python3
"""
localization_v14_footstep.launch.py  [Fase 1B — Odometry Realistis]
=====================================================================
IDENTIK dengan localization_v14.launch.py KECUALI:
  - kf_odom_node diganti footstep_odom_node
  - footstep_odom_node mengintegrasikan FootStepCommand.step_length/step_time
    dari /robotis/online_walking/foot_step_command → kecepatan yang di-command
...
TUJUAN: Fase 1B Gate G1 dengan sumber odom REALISTIS (tidak circular seperti GT)
  ...
  - Setelah run pertama: ukur k = GT_disp / odom_disp, set slip_ratio = 1/k
...
def generate_launch_description():
    ...
    # [FOOTSTEP] Ganti legged_odometry_kf dengan footstep_odom_node (Fase 1B realistis)
    # slip_ratio=1.0: default, kalibrasi dari GT setelah run pertama
    # Sumber: /robotis/online_walking/foot_step_command (bukan GT — tidak circular)
    kf_odom_node = Node(
        package="soccer_object_localization", executable="footstep_odom_node",
        name="footstep_odom", output="screen",
        parameters=[{
            "slip_ratio":  1.0,
            "zero_tf_yaw": True,
