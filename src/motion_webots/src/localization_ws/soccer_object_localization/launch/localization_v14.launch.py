#!/usr/bin/env python3
"""
localization_v14.launch.py  [v10.20]
=====================================
...
def generate_launch_description():
    ...
    # [FIX A] v3.2: absolute-height stance detection — /odom bergerak di Webots
    kf_odom_node = Node(
        ...
        parameters=[{"base_frame":"base_link","odom_frame":"odom","publish_rate":100.0,
                     ...
                     "zero_tf_yaw":True,"lift_thresh":0.003}]
    )
    ...
    amcl_node = Node(
        ...
        parameters=[{
            ...
            # [FIX G→B1] initial_pose.x=-0.363 (Webots spawn offset). Coupled dengan
            # ekf_soccer.yaml initial_state[-0.363,...] agar TF map→base_link konsisten.
            # FIX G revert ke 0.0 karena hanya setengah-fix (tanpa EKF offset). Sekarang
            # keduanya digeser bersamaan — TF konsisten, partikel di posisi benar sejak awal.
            "set_initial_pose":True,"initial_pose.x":-0.363,"initial_pose.y":0.0,
