#!/usr/bin/env python3
"""
localization_v14_walkparam.launch.py  [Fase 1B — Odometry Realistis via WalkingParam]
=======================================================================================
IDENTIK dengan localization_v14.launch.py KECUALI:
  - kf_odom_node diganti walking_param_odom_node
  - walking_param_odom_node mengintegrasikan x_move_amplitude/period_time
    dari /robotis/walking/set_params (op3_walking_module_msgs/WalkingParam)
  - Start/stop dari /robotis/walking/command (std_msgs/String)
  - IMU yaw dari /robotis_op3/imu (bukan GT)

MENGAPA BUKAN footstep_odom_node:
  - ball_follower.cpp menggunakan op3_walking_module (lama), BUKAN op3_online_walking_module
  - Ia publish ke /robotis/walking/set_params, BUKAN /robotis/online_walking/foot_step_command
  - footstep_odom_node menerima 0 pesan → odom_x = 0% selalu

TUJUAN: Fase 1B Gate G1 dengan sumber odom REALISTIS (tidak circular seperti GT)
  - Ada systematic error dari slip (slip_ratio=1.0 = no correction, kalibrasi dulu)
  - Setelah run pertama: ukur slip_ratio = GT_disp / odom_disp
...
def generate_launch_description():
    ...
    # [WALKPARAM] Ganti legged_odometry_kf dengan walking_param_odom_node (Fase 1B realistis)
    # Source: /robotis/walking/set_params + /robotis/walking/command
    # Ini yang dipakai ball_follower (op3_walking_module lama, bukan online_walking_module)
    kf_odom_node = Node(
        ...
        parameters=[{
            ...
            "param_timeout": 300.0,  # GUI publish set_params SEKALI per Apply — timeout besar
        }]
    )
